...
bool JT_BoBServer::take(const QDomElement &e)
{
	...
	if (data.attribute("xmlns") == "urn:xmpp:bob") {
		QDomElement iq = createIQ(doc(), "result", e.attribute("from"),
								  e.attribute("id"));
		iq.appendChild(client()->bobManager()
					   ->bobData(data.attribute("cid")).toXml(doc()));
		send(iq);
		return true;
	}
	return false;
...
void JT_BitsOfBinary::get(const Jid &j, const QString &cid)
{
	...
	BoBData bd = client()->bobManager()->bobData(cid);
	if (bd.isNull()) {
		d->iq = createIQ(doc(), "get", d->jid.full(), id());
