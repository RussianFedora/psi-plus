...
{
	QString text = QString::fromUtf8("» ") + getPlainText();
	text.replace("\n", QString::fromUtf8("\n» "));
	text.prepend("\n");
	text.append("\n");
	emit quote(text);
...
{
	if (mv.type() == MessageView::Message && share_.updateLastMsgTime(mv.dateTime())) {
		QString color = PsiOptions::instance()->getOption("options.ui.look.colors.messages.informational").toString();
...
void ChatView::renderMessage(const MessageView &mv)
{
	...
	if (mv.isEmote()) {
		...
		if (PsiOptions::instance()->getOption("options.ui.chat.use-chat-says-style").toBool()) {
			appendText(icon + QString("<span style=\"color: %1\">").arg(color) + QString("[%1] ").arg(timestr) + tr("%1 says:").arg(mv.nick()) + "</span><br>" + mv.formattedText());
		}
		else {
			appendText(icon + QString("<span style=\"color: %1\">").arg(color) + QString("[%1] &lt;").arg(timestr) + mv.nick() + QString("&gt;</span> ") + mv.formattedText());
		}
	}
