...
PsiTabWidget::PsiTabWidget(QWidget *parent)
	...
	if (PsiOptions::instance()->getOption("options.ui.tabs.can_close_inactive_tab") == QVariant::Invalid)
	    PsiOptions::instance()->setOption("options.ui.tabs.can_close_inactive_tab", true);
...
void PsiTabWidget::addTab(QWidget *widget, QString name, const QIcon &icon) {
	...
	if (PsiOptions::instance()->getOption("options.ui.tabs.show-tab-icons").toBool()) tabBar_->addTab(icon, name);
	else tabBar_->addTab(name);
	setLooks();
