...
void UserAccount::reset()
{
	...
	osname = SystemInfo::instance()->os();
	clientname = ApplicationInfo::name();
	clientversion = ApplicationInfo::version();
	capsnode = ApplicationInfo::capsNode();
	capsversion = ApplicationInfo::capsVersion();
	clientpreset = 1;
	ospreset = 1;
...
void UserAccount::fromOptions(OptionsTree *o, QString base)
{
	...
	if (PsiOptions::instance()->getOption("options.ui.account.mask.show") == QVariant::Invalid)
		PsiOptions::instance()->setOption("options.ui.account.mask.show", "false");

	if (PsiOptions::instance()->getOption("options.ui.account.mask.show").toBool())
	{
		ospreset = o->getOption(base + ".ospreset").toInt();
		clientpreset = o->getOption(base + ".clientpreset").toInt();
		if(ospreset != 1)
			osname = o->getOption(base + ".osname").toString();
		if(clientpreset != 1)
		{
			clientname = o->getOption(base + ".clientname").toString();
			clientversion = o->getOption(base + ".clientversion").toString();
			capsnode = o->getOption(base + ".capsnode").toString();
			capsversion = o->getOption(base + ".capsversion").toString();
		}
	}
}
...
void UserAccount::toOptions(OptionsTree *o, QString base)
{
	...
	if (PsiOptions::instance()->getOption("options.ui.account.mask.show").toBool())
	{
		o->setOption(base + ".clientpreset", clientpreset);
		o->setOption(base + ".ospreset", ospreset);
		if(ospreset != 1)
			o->setOption(base + ".osname", osname);
		if(clientpreset != 1)
		{
			o->setOption(base + ".clientname", clientname);
			o->setOption(base + ".clientversion", clientversion);
			o->setOption(base + ".capsnode", capsnode);
			o->setOption(base + ".capsversion", capsversion);
		}
	}
...
void UserAccount::fromXml(const QDomElement &a)
{
	...
	if (PsiOptions::instance()->getOption("options.ui.account.mask.show").toBool())
	{
		readEntry(a, "ospreset", (QString*)&ospreset);
		readEntry(a, "clientpreset", (QString*)&clientpreset);
		if(ospreset != 1)
			readEntry(a, "osname", &osname);
		if(clientpreset != 1)
		{
			readEntry(a, "clientname", &clientname);
			readEntry(a, "clientversion", &clientversion);
			readEntry(a, "capsnode", &capsnode);
			readEntry(a, "capsversion", &capsversion);
		}
	}
}
