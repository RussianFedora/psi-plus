...
QVariant PluginHost::getPluginOption(const QString& option, const QVariant& defValue)
{
	...
	QString optionKey=QString("%1.%2.%3").arg(PluginManager::pluginOptionPrefix).arg(shortName()).arg(option);
	QVariant value = PsiOptions::instance()->getOption(optionKey);
	if (value.isNull()){
		value = defValue;
	}
	return value;
...
QTextEdit* PluginHost::getEditBox(){
	...
	if (0 != chat){
		if (PsiOptions::instance()->getOption("options.ui.tabs.use-tabs").toBool() && PsiOptions::instance()->getOption("options.ui.tabs.grouping").toString().contains('A')){
			QSplitter* qs = chat->findChild<QSplitter*>("onewindowsplitter");
			if (PsiOptions::instance()->getOption("options.ui.contactlist.roster-at-left-when-all-in-one-window").toBool()){
				chat = qs->widget(1);
			} else {
				chat = qs->widget(0);
			}
		}
		QList<QWidget *> edit = chat->findChildren<QWidget *>("mle");
		if (!edit.isEmpty()){
            TabDlg* td = qobject_cast<TabDlg*>(chat);
            if (td == 0){
                QWidget* tab = edit.at(0);
                ed = (QTextEdit *)tab->children().at(1);
            } else {
                td->initFirstTab();
                TabbableWidget* tw = NULL;
				while((tw = td->getNextTab())){
                    if (tw->isActiveTab()){
                        QWidget* tab = tw->findChild<QWidget*>("mle");
                        if (tab != 0){
                            ed = (QTextEdit *)tab->children().at(1);
                        }
                        break;
	...
	if (0 != chat){
		...
		if (!edit.isEmpty()){
			...
			if (td != 0) {
				...
				GCMainDlg* cd = qobject_cast<GCMainDlg*>(chat); // режим без табов, конференция
				if (cd != 0){
					jid = cd->jid().full();
				} else {
					ChatDlg* cd = qobject_cast<ChatDlg*>(chat); // режим без табов, чат
					if (cd != 0) {
						jid = cd->jid().full();
					} else {
						QList<QWidget *> tmp = chat->findChildren<QWidget *>(); // все-в-одном
						while(!tmp.isEmpty()) {
							TabDlg* td = qobject_cast<TabDlg*>(tmp.takeFirst());
							if (td != 0) {
								td->initFirstTab();
								TabbableWidget* tw = NULL;
								while((tw = td->getNextTab())){
...
bool PluginHost::setActivity(int account, QString Jid, QDomElement xml) {
	return manager_->setActivity(account, Jid, xml);
}

bool PluginHost::setMood(int account, QString Jid, QDomElement xml) {
	return manager_->setMood(account, Jid, xml);
}
bool PluginHost::setTune(int account, QString Jid, QString tune) {
...
void PluginHost::addToolBarButton(QObject* parent, QWidget* toolbar, int account, const QString& contact)
{
	...
	if (ta) {
		...
		for (int i = 0; i < buttons_->size(); ++i) {
				...
				connect(button, SIGNAL(triggered()),param.value("reciver").value<QObject *>(), param.value("slot").value<QString>());
...
void PluginHost::initPopup(QString text, QString title, QString icon) {
...
void PluginHost::addAccountMenu(QMenu *menu, int account)
{
	...
	if(ma && accMenu_ != 0) {
		for (int i = 0; i < accMenu_->size(); ++i) {
			...
			connect(act, SIGNAL(triggered()), param.value("reciver").value<QObject *>(), param.value("slot").value<QString>());
			menu->addAction(act);
		}
	}
}

void PluginHost::addContactMenu(QMenu *menu, int account, QString jid)
{
	MenuAccessor *ma = qobject_cast<MenuAccessor*>(plugin_);
	if(ma && contactMenu_ != 0) {
...
void PluginHost::setStatus(int account, QString status, QString statusMessage)
{
