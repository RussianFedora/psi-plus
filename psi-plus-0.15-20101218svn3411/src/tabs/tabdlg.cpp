...
{
	if (tab->name() == QString("GroupChatDlg")) {
		saveSize();
...
void TabDlg::mouseMiddleClickTab(QWidget* widget) {
	if(PsiOptions::instance()->getOption("options.ui.tabs.mouse-middle-button").toString() == "close")
		closeTab(static_cast<TabbableWidget*>(widget));
	else hideTab(static_cast<TabbableWidget*>(widget));
}
...
void TabDlg::checkHasChats()
{
	if (window() != this) {
		...
		return;
	} else if (tabWidget_->count() > 0) return;
...
void TabDlg::updateTab(TabbableWidget* chat)
{
	...
	case TabbableWidget::StateUnread:
		tabWidget_->setTabIcon(chat, IconsetFactory::iconPtr("psi/chat")->icon());
		break;

	case TabbableWidget::StateUnreadToMe:
...
if (this->currentTab == i)
    this->closeTab(static_cast<TabbableWidget*>(tabWidget_->page(i)));
else
    if (!PsiOptions::instance()->getOption("options.ui.tabs.can_close_inactive_tab").toBool())
	this->tabSelected(tabWidget_->page(i));
    else
	this->closeTab(static_cast<TabbableWidget*>(tabWidget_->page(i)));
}

void TabDlg::initFirstTab(){
	currentTab = -1;
}

TabbableWidget* TabDlg::getNextTab(){
	currentTab = currentTab + 1;
	if ( currentTab < tabWidget_->count() ){
		return static_cast<TabbableWidget*>(tabWidget_->page(currentTab));
	} else {
		return NULL;
	}
}
