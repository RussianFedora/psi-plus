...
void GCUserViewItem::paintCell(QPainter *p, const QColorGroup & cg, int column, int width, int alignment) 
{
	QColorGroup xcg = cg;
	MUCItem::Role r = s.mucItem().role();

	if(PsiOptions::instance()->getOption("options.ui.muc.roster-nick-coloring").toBool()) {
		if(r == MUCItem::Moderator)
			xcg.setColor(QColorGroup::Text, PsiOptions::instance()->getOption("options.ui.look.colors.muc.role-moderator").value<QColor>());
		else if(r == MUCItem::Participant)
			xcg.setColor(QColorGroup::Text, PsiOptions::instance()->getOption("options.ui.look.colors.muc.role-participant").value<QColor>());
		else if(r == MUCItem::Visitor)
			xcg.setColor(QColorGroup::Text, PsiOptions::instance()->getOption("options.ui.look.colors.muc.role-visitor").value<QColor>());
		else
			xcg.setColor(QColorGroup::Text, PsiOptions::instance()->getOption("options.ui.look.colors.muc.role-norole").value<QColor>());
	}

	Q3ListViewItem::paintCell(p, xcg, column, width, alignment);

	QList<QPixmap> rightPixs;
	if(PsiOptions::instance()->getOption("options.ui.muc.show-client-icons").toBool()) {
		GCUserView *gcuv = (GCUserView*)listView();
		GCMainDlg* dlg = gcuv->gcDlg_;
		QPixmap clientPix;
		if(dlg) {
			UserListItem u;
			const QString &nick = text(0);
	...
	if(PsiOptions::instance()->getOption("options.ui.muc.show-affiliation-icons").toBool()) {
		MUCItem::Affiliation a = s.mucItem().affiliation();
		QPixmap pix;
		if(a == MUCItem::Owner)
	...
	/*QColor bgc = isSelected() ? xcg.color(QPalette::Highlight) : xcg.color(QPalette::Base);
	QColor tbgc = bgc;
	tbgc.setAlpha(0);
	int sumWidth = 0;
	...
	QBrush tbakBr(grad);
	QRect gradRect(0,0, width, height());
	gradRect.setLeft(width - sumWidth - 20);
	p->fillRect(gradRect, tbakBr); */

	for (int i=0; i<rightPixs.size(); i++) {
		const QPixmap pix = rightPixs[i];
		int pixWidth = pix.width();
		int x = width - pixWidth - 1;
		int y = (height() - pix.height()) / 2;
		p->drawPixmap(x, y, pix);
		width = width - pixWidth -1;
	}
}
...
void GCUserViewGroupItem::setHeight(int height)
{
	if(!PsiOptions::instance()->getOption("options.ui.muc.show-groups").toBool())
		Q3ListViewItem::setHeight(0);
	else
		Q3ListViewItem::setHeight(height);
}
...
bool GCUserView::eventFilter(QObject *obj, QEvent *e)
{
	if (obj == viewport() && e->type() == QEvent::Resize) {
		setColumnWidth(0, viewport()->size().width());
		return true;
	} 
...
void GCUserView::qlv_contextMenuRequested(Q3ListViewItem *i, const QPoint &pos, int)
{
	...
	if (AvCallManager::isSupported()) {
		pm->insertItem(IconsetFactory::icon("psi/avcall").icon(), tr("Voice Call"), 5);
	}
	pm->insertSeparator();
	...
	int x = pm->exec(pos);
	bool enabled = pm->isItemEnabled(x) || rm->isItemEnabled(x) || am->isItemEnabled(x) || kickMenu->isItemEnabled(x) || banMenu->isItemEnabled(x);
	delete pm;
...
void GCUserView::qlv_mouseButtonClicked(int button, Q3ListViewItem* item, const QPoint& pos, int c)
{
	...
	if (!(button == Qt::MidButton ||
		(button == Qt::LeftButton && qApp->keyboardModifiers() == Qt::ShiftModifier)))
		return;
