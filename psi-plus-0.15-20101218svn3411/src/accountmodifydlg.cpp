...
void AccountModifyDlg::init() 
{
	...
	le_osname->setText(acc.osname);
	le_clientname->setText(acc.clientname);
	le_clientversion->setText(acc.clientversion);
	le_capsnode->setText(acc.capsnode);
	le_capsversion->setText(acc.capsversion);

	for(int i=0; i<OS_PRESET; i++)
		cb_ospreset->addItem(ospresetname[i], i);
	connect(cb_ospreset, SIGNAL(activated(int)), SLOT(ospresetActivated(int)));

	for(int i=0; i<CLIENTS_PRESET; i++)
		cb_clientpreset->addItem(clientpreset[i].name, i);
	connect(cb_clientpreset, SIGNAL(activated(int)), SLOT(clientpresetActivated(int)));

	cb_ospreset->setCurrentIndex(cb_ospreset->findData(acc.ospreset));
	cb_clientpreset->setCurrentIndex(cb_clientpreset->findData(acc.clientpreset));

	if(cb_ospreset->itemData(cb_ospreset->currentIndex()).toInt() != OS_USERDEF_PRESET)
		le_osname->setDisabled(true);

	if(cb_clientpreset->itemData(cb_clientpreset->currentIndex()).toInt() != CLIENT_USERDEF_PRESET)
	{
		le_clientname->setDisabled(true);
		le_clientversion->setDisabled(true);
		le_capsnode->setDisabled(true);
		le_capsversion->setDisabled(true);
	}
	...
	if (!PsiOptions::instance()->getOption("options.ui.account.mask.show").toBool())
		tab_main->removeTab(tab_main->indexOf(tab_mask));
...
void AccountModifyDlg::save()
{
	...
	acc.osname = le_osname->text();
	acc.clientname = le_clientname->text();
	acc.clientversion = le_clientversion->text();
	acc.capsnode = le_capsnode->text();
	acc.capsversion = le_capsversion->text();
	acc.clientpreset =  cb_clientpreset->itemData(cb_clientpreset->currentIndex()).toInt();
	acc.ospreset =  cb_ospreset->itemData(cb_ospreset->currentIndex()).toInt();
...
void AccountModifyDlg::clientpresetActivated(int i)
{
	if (i==CLIENT_USERDEF_PRESET)
	{
		le_clientname->setText(acc.clientname);
		le_clientversion->setText(acc.clientversion);
		le_capsnode->setText(acc.capsnode);
		le_capsversion->setText(acc.capsversion);
		le_clientname->setDisabled(false);
		le_clientversion->setDisabled(false);
		le_capsnode->setDisabled(false);
		le_capsversion->setDisabled(false);
	}
	else
	if (i==CLIENT_DEFAULT_PRESET)
	{
		le_clientname->setText(ApplicationInfo::name());
		le_clientversion->setText(ApplicationInfo::version());
		le_capsnode->setText(ApplicationInfo::capsNode());
		le_capsversion->setText(ApplicationInfo::capsVersion());
		le_clientname->setDisabled(true);
		le_clientversion->setDisabled(true);
		le_capsnode->setDisabled(true);
		le_capsversion->setDisabled(true);
	...
	{
		le_clientname->setText(clientpreset[i].name);
		le_clientversion->setText(clientpreset[i].version);
		le_capsnode->setText(clientpreset[i].capsnode);
		le_capsversion->setText(clientpreset[i].capsver);
		le_clientname->setDisabled(true);
		le_clientversion->setDisabled(true);
		le_capsnode->setDisabled(true);
		le_capsversion->setDisabled(true);
...
void AccountModifyDlg::ospresetActivated(int i)
{
	le_osname->setDisabled(true);
	if( i == OS_USERDEF_PRESET )
	{
		le_osname->setText(acc.osname);
		le_osname->setDisabled(false);
	}
	else
	if( i == OS_DEFAULT_PRESET )
	{
		le_osname->setText(SystemInfo::instance()->os());
	}
	else
	{
		le_osname->setText(ospresetname[i]);
	}
}
