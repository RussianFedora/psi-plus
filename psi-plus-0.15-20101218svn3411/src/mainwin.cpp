...
const QString rosterGeometryPath      = "options.ui.save.roster-geometry";
const QString tabsGeometryPath        = "options.ui.save.tabs-geometry";
...
:AdvancedWidget<QMainWindow>(0, (_onTop ? Qt::WindowStaysOnTopHint : Qt::Widget) | (_asTool ? (Qt::Tool |TOOLW_FLAGS) : Qt::Widget))
{
	...
	if ( PsiOptions::instance()->getOption("options.ui.tabs.use-tabs").toBool() && PsiOptions::instance()->getOption("options.ui.tabs.grouping").toString().contains('A')) {
		...
		d->rosterGeometry = PsiOptions::instance()->getOption(rosterGeometryPath).toSize();
		d->tabsGeometry = PsiOptions::instance()->getOption(tabsGeometryPath).toSize();
		rosterBar->resize(d->rosterGeometry);
		d->mainTabs->resize(d->tabsGeometry);
	} else setCentralWidget(rosterBar);
...
MainWin::~MainWin()
{
	...
	if(d->splitter != NULL) {
		PsiOptions::instance()->setOption(rosterGeometryPath, d->rosterGeometry);
		PsiOptions::instance()->setOption(tabsGeometryPath, d->tabsGeometry);
	}

	delete d;
}

void MainWin::splitterMoved(int pos, int index)
{
	int rosterIdx = 1;
	if (PsiOptions::instance()->getOption("options.ui.contactlist.roster-at-left-when-all-in-one-window").toBool())
		rosterIdx = 0;

	QWidget * rosterBar = d->splitter->widget(rosterIdx);
	if (rosterBar && d->mainTabs) {
		d->rosterGeometry = rosterBar->size();
		d->tabsGeometry = d->mainTabs->size();
	}
}
