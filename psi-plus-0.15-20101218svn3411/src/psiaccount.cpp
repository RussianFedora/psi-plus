	...
	: QObject(parent)
{
	...
	d->client = new Client;
	d->client->setOSName(acc.osname);
	d->client->setClientName(acc.clientname);
	d->client->setClientVersion(acc.clientversion);
	d->client->setCapsNode(acc.capsnode);
	d->client->setCapsVersion(acc.capsversion);
	d->client->bobManager()->setCache(BoBFileCache::instance()); // xep-0231

	DiscoItem::Identity identity;
	identity.category = "client";
	identity.type = "pc";
	identity.name = acc.clientname;
...
//			isAuthError = true;
			...
			s = tr("Temporary auth failure or wrong password");
			badpass = true;
//			isAuthError = true;
...
void PsiAccount::passwdPrompt()
{ 
	Jid j = d->jid;
	
	bool ok = false;
	bool savepassword = (d->acc.opt_pass);
	QString text = "";
	
	PassDialog *dialog = new PassDialog(0, j.full(), &ok, &savepassword, &text);
	dialog->exec();
...
void PsiAccount::addMucItem(const Jid& jid)
{
	...
	if (!d->groupchats.contains(jid.bare())) d->groupchats += jid.bare();
...
void PsiAccount::setNewInfo(UserAccount acc)
{
	d->client->setOSName(acc.osname);
	d->client->setClientName(acc.clientname);
	d->client->setClientVersion(acc.clientversion);
